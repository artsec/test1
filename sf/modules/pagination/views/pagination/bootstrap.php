<?php
    ...
    $count_out = (!empty($config['count_out']))
        ? (int)$config['count_out']
        : 3;
    // Number of page links on each side of current page
    $count_in = (!empty($config['count_in']))
        ? (int)$config['count_in']
        : 5;
...
<ul class="pagination pagination-sm">
    <?php foreach ($links as $number => $content): ?>
        ...
        <?php if ($number === $current_page): ?>
            <li class="active"><a href="#"><?= $content ?> <span class="sr-only">(current)</span></a></li><strong></strong>
        <?php else: ?>
